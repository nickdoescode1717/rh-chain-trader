...
export function formatPositionDetail(p: Position): FormattedMessage {
  ...
  const route = p.entrySnapshot?.execution?.model?.version === "pons-route-paper-v1" ? p.entrySnapshot.execution : null;
  return { text: ["📋 POSITION · PAPER", "", ...positionLines(p), "", `Entry price  ${amount(m.entry)}${p.entrySnapshot ? ` ${p.entrySnapshot.currency}` : ""}`, `Recorded price  ${amount(m.mark)}${p.entrySnapshot ? ` ${p.entrySnapshot.currency}` : ""}`,
    ...
    ...(route ? [`Entry model  Pons V2 route simulation`, `Buy fee + tax  ${amount(route.fee)} ETH`, `Charged gas allowance  ${amount(route.gasAllowance)} ETH`, `Simulated buy gas  ${amount(route.buyGasEstimate)} ETH (L1 fee excluded)`, "Exits request fresh exact Pons curve reserves, base fee and creator tax; exit gas is excluded."] : []),
