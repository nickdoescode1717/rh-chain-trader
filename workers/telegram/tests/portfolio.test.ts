...
test("route-priced positions disclose fee, allowance and exit-model limits",()=>{
 ...
 const card=formatPositionDetail(p);assert.match(card.text,/Pons V2 route simulation/);assert.match(card.text,/Charged gas allowance  0.0001 ETH/);assert.match(card.text,/L1 fee excluded/);assert.match(card.text,/fresh exact Pons curve reserves/);
});
