...
- 134 local tests and seven isolated PostgreSQL scripts passed, including automatic queue deduplication, no fallback, stale/changed identity, price/loss/gas limits, rollback, concurrent/restart exactly-once settlement, partial-exit cost allocation and legacy regression. Production preserved 0.88 ETH, three holdings and all state. Zero plans created; collection remains STOPPED, chain OFF and live flags false. Backup `/root/rh-deploy-backups/20260916T135848Z-route-paper`, marker `route-paper-current`; helpers `rh-route-paper-{build-test,cutover,verify}.py` under Desktop Files/Bot Operations. See `docs/deployments/2026-09-16-route-paper.md`.
