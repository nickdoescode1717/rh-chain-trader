...
**Route-derived paper settlement (deployed 2026-09-16 UTC)**
- Application `54f1f380d21aa72dafe9be77f159a940018f3687` is deployed to API/collector/Telegram. New `/snipe` drafts are immutable policy v2: spend plus a visible fixed gas allowance (default 0.0001 ETH) is reserved, and entry requires a fresh identity-bound native-ETH Pons V2 buy/sell simulation. DEX indexing is not required. Exact route quantity/fees/tax drive the fill; maximum gross unit price, 10% immediate round-trip-loss ceiling and estimated buy-gas allowance are enforced. No fixed-model fallback. Existing version-1 approvals retain their original $1,000 indexed-liquidity and fixed-fee policy.
- Route report binding includes the plan, reviewed source hash/time, deployment transaction/block and request time. Settlement rechecks identity/freshness under the book lock. Paper accounting charges the full approved gas allowance and discloses that L1 fees are unmeasured. P&L remains unavailable until market data arrives; manual exits still use the old reference-price model. Other venues, route-based exits and live signing remain missing.
- 134 local tests and seven isolated PostgreSQL scripts passed, including automatic queue deduplication, no fallback, stale/changed identity, price/loss/gas limits, rollback, concurrent/restart exactly-once settlement, partial-exit cost allocation and legacy regression. Two strengthened settlement suites then passed again in fresh databases against the deployed code. Production preserved 0.88 ETH, three holdings and all state. Zero plans created; collection remains STOPPED, chain OFF and live flags false. Backup `/root/rh-deploy-backups/20260916T135848Z-route-paper`, marker `route-paper-current`; helpers `rh-route-paper-{build-test,cutover,verify}.py` plus `rh-route-paper-final-tests.py` under Desktop Files/Bot Operations. See `docs/deployments/2026-09-16-route-paper.md`.

**Verified route diagnostics (deployed 2026-09-15 UTC)**
- Application `9ce5a1d58671df3582f70b8debce678fc54e7000` deployed to API/collector/Telegram; additive migration 0016. `/snipes` plan cards now offer **Test buy + sell**. Telegram-owner-only requests require a fresh reviewed exact identity and enabled collection; durable bounded jobs run a native-ETH Pons V2 curve quote and sequential `eth_simulateV1` round trip. Pinned verified factory/curve/token bytecode, fixed unfunded hypothetical wallet, balance/event validation, expiry/reorg checks, active-launch-tax rejection and up-to-20-request budget. See docs/ROUTE_CHECKS.md.
- **Diagnostic only:** route results do not gate automatic paper entries or replace the current fixed-fee reference-price ledger model. No signing, funding, broadcast or exit automation. Next priority is an explicitly described route-based paper execution policy with realistic quantities/costs/gas limits, followed by isolated live signing. ERC-20 quote assets and graduated V4 pools need separate adapters.
- 128 local tests and six isolated PostgreSQL scripts passed. A real mainnet round trip passed twice without broadcast; the final guarded revision used 17 public RPC calls at parent block 63994953. Separate unsupported ERC-20 routing was correctly rejected. Production stayed paused and used no new RPC/X calls during cutover; 0.88 ETH, three holdings, history, claims, settings and cursors preserved. Zero watches or snipe plans created. Read-only deployed Telegram rendering/auth checks passed. Do not resume collection automatically.
- Latest backup `/root/rh-deploy-backups/20260915T220615Z-route`, marker `route-current`; helpers `rh-route-{build-test,cutover,verify}.py` under Desktop Files/Bot Operations. Previous running application was `ec03c76211de0ce2cf76196e195b1da18fcf2ce9`; a failed initial staging build means the backup's checkout hash may differ from its running image tags. Restore by recorded image tags, keep additive schema, never restore old accounting over new fills. See docs/deployments/2026-09-15-route-checks.md.

**Longer launch watches and clearer sniper setup (deployed 2026-09-15 UTC)**
- Application `ec03c76211de0ce2cf76196e195b1da18fcf2ce9` deployed to API/collector/Telegram; no new migration. New paper plans allow 1–720 hours (default 24); existing immutable approvals keep their terms. Targeted Pons V2 checks batch due deployers every ~10 seconds initially, then ~5 minutes until expiry. Durable leases/cursors, bounded range fallback and catch-up remain within the shared RPC cap. A research `/launch` watch has no scheduled expiry while enabled; it never authorizes a trade.
- `/snipe` and `/snipe @handle` now explain each input, total budget versus price per token, an explicit placeholder example, unknown-deployer/date workflow and Telegram arming/source review. Cards explain blocking reasons and next actions. See docs/PAPER_SNIPER.md and docs/LIVE_EXECUTION_NEXT.md; live route quotes/simulation then isolated signing are the next priorities.
- 123 local tests, five isolated PostgreSQL scripts and deployed read-only Telegram handler checks passed. 0.88 ETH/three holdings and all stored watches/claims/cursors preserved; no plans created. Collection remains STOPPED, chain OFF, live flags false, zero new X/RPC calls observed. Do not resume automatically. Real launch/provider validation and live execution remain outstanding.
- Backup `/root/rh-deploy-backups/20260915T170943Z-snipe`, marker `snipe-sept15-current`; local helpers `rh-snipe-sept15-{build-test,cutover,verify}.py` under Desktop Files/Bot Operations. See docs/deployments/2026-09-15-snipe-monitor.md.
