...
watchRoutes.post("/", async c => {
  ...
  if (!isRecord(body) || typeof body.input !== "string" || (body.launch !== undefined && body.launch !== true)
    || Object.keys(body).some(k => !["input", "actor", "launch"].includes(k))) return c.json({ error: "invalid_watch_input" }, 400);
  ...
  const data = await db.transaction(async tx => {
    ...
    const [saved] = await tx.insert(watchTargets).values({ inputKey: parsed.key, handle: parsed.handle, domain: parsed.domain, projectHandle,
      launchFlag: body.launch === true, enabled: body.launch && project ? project.enabled : true })
      .onConflictDoUpdate({ target: watchTargets.inputKey, set: body.launch ? { launchFlag: true,
        revision: sql`case when ${watchTargets.launchFlag} then ${watchTargets.revision} else ${watchTargets.revision} + 1 end` }
        : { enabled: true, revision: sql`case when ${watchTargets.enabled} then ${watchTargets.revision} else ${watchTargets.revision} + 1 end` } }).returning();
    return saved;
  });
  return c.json({ data }, 201);
});
watchRoutes.post("/:id/launch", async c => {
  const body = await c.req.json().catch(() => null);
  const auth = telegramDecisionError(c.req.header("x-telegram-approval-token"), body?.actor);
  if (auth) return c.json({ error: auth.error }, auth.status);
  ...
  const data = await db.transaction(async tx => {
    await tx.execute(sql`select pg_advisory_xact_lock(4663, 9011)`);
    const [row] = await tx.update(watchTargets).set({ launchFlag: body.enabled as boolean,
      revision: sql`case when ${watchTargets.launchFlag} = ${body.enabled} then ${watchTargets.revision} else ${watchTargets.revision} + 1 end` })
