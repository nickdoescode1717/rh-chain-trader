...
- After deployment, the strengthened version-1 and policy-v2 scripts passed again in two new disposable databases mounted over the unchanged deployed application image. This specifically retained the malformed-policy, deployment-order and round-trip-loss assertions. Production remained paused and recorded zero new RPC calls.
