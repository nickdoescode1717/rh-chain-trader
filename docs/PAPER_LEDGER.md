...
Policy-v2 Pons positions use a durable two-quote exit flow. The first canonical-block reserve quote supplies the Telegram preview and minimum proceeds. Telegram confirmation records approval and requests a second quote; only the fresh second report can settle. Both reports validate the pinned factory, curve and token bytecode, creator, native quote asset, 18-decimal token, curve token balance, ungraduated state, reserves, base fee and creator tax. The fill includes exact curve price impact and fees. Exit gas is not yet charged, so it remains a stated paper-model limitation.
