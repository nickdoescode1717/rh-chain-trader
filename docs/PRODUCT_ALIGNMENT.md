...
## Automatic paper launch entries (2026-09-11 UTC)
See [PAPER_SNIPER.md](PAPER_SNIPER.md). Telegram can draft and explicitly arm immutable one-entry paper plans with reserved ETH, a unit-price ceiling, liquidity floor and expiry. An exact newly deployed mainnet identity plus fresh eligible quote can trigger one atomic paper fill; stop cancels armed plans. The September 15 update extends new approved durations to 30 days and batches targeted Pons V2 deployer checks: ten-second ticks initially, five-minute checks after ten minutes, within the existing RPC guard. `/snipe` now explains every input and the next action for blocked plans. This supersedes the earlier statement that all execution policies/targeted monitoring are missing. It does not implement live signing, actual router execution, contract safety/sellability simulation, other launchpad adapters or first-block execution. Grok's testnet scout remains an external lead source; a testnet CA is never a mainnet execution target.

## Read-only route assessment (2026-09-15)
...
New Telegram-approved policy-v2 plans require a fresh Pons V2 native-ETH simulation before entry. DEX indexing is not required. The result is bound to the immutable limits and reviewed identity; exact simulated quantity/fees/tax drive the fill, while price, immediate round-trip-loss and buy-gas limits apply. Spend plus a fixed approved gas allowance is reserved and charged. Manual exits use fresh exact Pons reserves and fee terms before preview and again after approval, with no fixed-model fallback. Automatic route retries are bounded. Old approvals remain version 1. Other venues, graduated pools, exit-gas measurement, automatic exit policies and live signing remain outstanding.
