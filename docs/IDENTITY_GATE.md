...
- Supported deployment paths: direct contract creation, and the explicitly configured Pons V2 factory's `TokenLaunched` event with exact emitter/token/deployer and historical code checks. Pons calls may pass through a router/multisig; the authenticated log emitter is checked independently of the transaction destination. Arbitrary factory logs, pool creation, other factories and unsupported archive queries remain unverified. An event from an unrelated emitter cannot prove identity.
