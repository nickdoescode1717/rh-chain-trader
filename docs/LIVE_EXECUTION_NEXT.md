...
As of September 16, the first adapter below and its route-derived policy-v2 paper entry and manual exit are implemented for native-ETH Pons V2 curves. Fresh entry reports are bound to Telegram-approved limits and reviewed identity; exact quantities/fees/tax plus a fixed entry gas allowance are recorded. Manual exits use current verified curve reserves, reserve impact, base fee and creator tax, with a second fresh check after Telegram confirmation. DEX indexing is no longer required for these fills. The remaining work is to prove the complete launch path and build isolated signing/recovery before any live enablement.
